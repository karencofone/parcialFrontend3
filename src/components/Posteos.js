...
export default function Posteos({likes, setLikes, aumentar}) {
  return (
    <div className='container'>
      {data.map(item => <Post key={item.id} titulo={item.titulo} texto={item.texto}
      likes={likes}setLikes={setLikes} aumentar={aumentar}/>)}
    </div>
  )
}
